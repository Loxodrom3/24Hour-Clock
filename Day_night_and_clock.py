...
numLED = 60
ORDER = neopixel.GRB
ringLED = neopixel.NeoPixel(LEDPin, numLED, brightness=0.1, auto_write=False, pixel_order=ORDER)
...
while True:
    ...
    dayOfYear = monthDay[current.tm_mon] + current.tm_mday
    if toDay != dayOfYear:
        toDay = dayOfYear
        setPixel = int((ring1*sunSet[toDay]/base) - (ring1 / 2))
        risePixel = int((ring1*sunRise[toDay]/base) + (ring1 / 2)) -1 # should use ceiling
        # risePct =
        # setPct =
